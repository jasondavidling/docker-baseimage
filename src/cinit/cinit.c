...
#define ASSERT_VALID_SERVICE_INDEX(sid) assert(sid >= 0 && sid < DIM(g_ctx.services))
...
typedef struct {
    ...
    int last_to_start;                    /**< Last service that should be started. */
...
static int load_service(const char *service)
{
    ...
    // Make sure the run file exists.
    if (access("run", F_OK) != 0) {
        ThrowMessage("run file not found");
    }
    ...
    // Create the logger.
    if (pthread_create(&SRV(sid).logger, NULL, service_logger, (void*)(intptr_t)sid) == 0) {
...
static void load_service_with_deps(const char *service, int dependent)
{
    ...
    Try {
        ...
        load_value_as_bool("disabled", &disabled);
        if (disabled) {
            log("skipping disabled service '%s'.", service);
            ExitTry();
        }

        if (access("run", X_OK) != 0) {
            // Do not add a service group (service without the run program) to our
            // table.  We just need to load its dependencies.
            log("handling service group '%s'...", service);
        }
        else {
            ...
            add_to_start_order(sid, dependent < 0 ? g_ctx.last_to_start : dependent);

            log_debug("service '%s' successfully loaded.", service);
        }
        ...
        // Load dependencies.
        {
            ...
            if (!dirstream) {
                ThrowMessageWithErrno("could not open stream for service directory");
            }
            ...
            while ((dir = readdir(dirstream)) != NULL) {
                ...
                *p = '\0';
                ...
                if (strcmp(dir->d_name, "*all*") == 0) {
                    // Make sure there is no another services depending on
                    // everything.
                    if (g_ctx.last_to_start != -1) {
                        ThrowMessage("service '%s' already identified as depending on all",
                                SRV(g_ctx.last_to_start).name);
                    }

                    // Save the last service to start.
                    g_ctx.last_to_start = sid;

                    // Make sure the loaded service is the last to start.
                    for (int i = 0; i < DIM(g_ctx.start_order); i++) {
                        if (g_ctx.start_order[i] == -1) {
                            assert(i != 0);
                            if (g_ctx.start_order[i - 1] != sid) {
                                ThrowMessage("circular dependency detected: "
                                    "no service should depend on '%s'",
                                    SRV(sid).name);
                            }
                            break;
                        }
                    ...
                    // No need to process more dependencies.
                    break;
                }
                else {
                    // Load service.
...
static void unload_services()
{
    // Stop the loggers.
    FOR_EACH_SERVICE(sid) {
        if (SRV(sid).logger_started) pthread_cancel(SRV(sid).logger);
    }
    FOR_EACH_SERVICE(sid) {
        if (SRV(sid).logger_started) pthread_join(SRV(sid).logger, NULL);
    }
...
int main(int argc, char *argv[])
{
    ...
    g_ctx.log_prefix_length = strlen(g_ctx.progname);

    // Initialize start order of services.
    for (int i = 0; i < DIM(g_ctx.start_order); ++i) {
        g_ctx.start_order[i] = -1;
    }
    g_ctx.last_to_start = -1;
    ...
    while (true) {
        ...
        FOR_EACH_SERVICE(sid) {
            if (!SRV(sid).respawn && SRV(sid).interval > 0) {
